...
public class WumpusWorldAgent extends Agent {
    ...
    protected void setup() {
        System.out.println("The Wumpus world agent " + getAID().getName() + " is ready.");
        ...
        addBehaviour(new SpeleologistConnectRequest());
    ...
    protected void takeDown() {
        ...
        System.out.println("The Wumpus world agent " + getAID().getName() + " is terminated.");
    }

    private class SpeleologistConnectRequest extends CyclicBehaviour {
        @Override
        public void action() {
        ...
        public void action() {
            ...
            if (msg != null) {
                ...
                if (message.equals(Constants.GAME_INFORMATION)) {
                    ...
                    WumpusPercept percept_class = getPerceptOfSpeleologist();
                    String percept_string = percept_class.toString();
                    reply.setContent(percept_string);
                    myAgent.send(reply);
                }
            } else {
                block();
    ...
    private void turnLeft() {
        agentPosition.turnLeft();
    }

    private void turnRight() {
        agentPosition.turnRight();
    }

    private boolean moveForward() {
        agentPosition.moveForward();
    ...
    private void shoot() {
        cave.getWumpus();
        if (hasArrow && isArrowFacingWumpus(agentPosition)) {
            isWumpusAlive = false;
    ...
    private boolean isArrowFacingWumpus(AgentPosition pos) {
    ...
    public WumpusPercept getPerceptOfSpeleologist() {
        WumpusPercept result = new WumpusPercept();
        
        List<Room> adjacentRooms = Arrays.asList(
                new Room(agentPosition.getX()-1, agentPosition.getY()), new Room(agentPosition.getX()+1, agentPosition.getY()),
                new Room(agentPosition.getX(), agentPosition.getY()-1), new Room(agentPosition.getX(), agentPosition.getY()+1)
        );
        ...
        if (agentPosition.getRoom().equals(cave.getGold()))
            result.setGlitter();
        if (!isWumpusAlive)
            result.setScream();
